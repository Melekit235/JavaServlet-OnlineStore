...
<jsp:useBean id="cart" scope="session" type="com.bsuir.verghel.basketball.model.entities.cart.Cart"/>
...
<tags:master pageTitle="Cart">
            ...
            <c:if test="${cart.items.size() > 0}">
                    ...
                    <table class="table table-hover table-bordered text-center">
                        ...
                        <c:forEach var="cartItem" items="${cart.items}">
                            <tr>
                                <td>${cartItem.ball.brand}</td>
                                <td>${cartItem.ball.model}</td>
                                <td>$ ${cartItem.ball.price}</td>
                                <td>
                                    <input type="number" name="quantity" value="${cartItem.quantity}" min="1" required>
                                    <c:if test="${not empty inputErrors.get(cartItem.ball.id)}">
                                        <div class="error" style="color: red">${inputErrors.get(cartItem.ball.id)}</div>
                                    </c:if>
                                </td>
                                <td>
                                    <input type="hidden" name="id" value="${cartItem.ball.id}">
                                    <button class="btn btn-danger" type="button" onclick="deleteCartItem(${cartItem.ball.id})"><fmt:message key="button_delete" /></button>
                                </td>
                            </tr>
                        </c:forEach>
                    </table>
    ...
    <script>
        ...
        function deleteCartItem(ballId) {
